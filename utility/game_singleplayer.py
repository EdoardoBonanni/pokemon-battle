...
def checks_2_turns_attack(BattleWindow, move_me, move_enemy):
    ...
    if BattleWindow.special_moves_me and BattleWindow.special_moves_enemy:
        check_turn_type(BattleWindow, BattleWindow.special_moves_me, BattleWindow.special_moves_enemy)
    elif BattleWindow.special_moves_me:
        check_turn_type(BattleWindow, BattleWindow.special_moves_me, move_enemy)
    elif BattleWindow.special_moves_enemy:
        check_turn_type(BattleWindow, move_me, BattleWindow.special_moves_enemy)
    else:
        check_turn_type(BattleWindow, move_me, move_enemy)


def check_turn_type(BattleWindow, move_me, move_enemy):
    """
    ...
    # if None the Pokemon in battle is not fainted and changed from game.
    if move_me is None:
        ...
        classic_turn(BattleWindow, move_me, move_enemy)


def classic_turn(BattleWindow, move_me, move_enemy):
    """
...
def determine_first_attacker(BattleWindow, move_me, move_enemy):
    ...
    if move_me.name == 'ExtremeSpeed' and move_enemy.name != 'ExtremeSpeed':
        ...
        if move_me.name == 'Quick Attack' and move_enemy.name != 'Quick Attack':
            ...
            if BattleWindow.model.me.team[0].battleSpeed >= BattleWindow.model.enemy.team[0].battleSpeed:
                first_attack_me = True
            else:
                first_attack_me = False
...
def attack(BattleWindow, attacker_model, defender_model, move, player_attacker, count_move_attacker,
           special_moves_defender,
           count_move_defender):
    """
    ...
    if prob > move_accuracy or always_miss_attack:
                ...
                if special_moves_defender:
                    if special_moves_defender.name.lower() != 'dig' and count_move_defender != 1 and move.name.lower() != 'earthquake':
                        show_screen_elements.explosion_animations(BattleWindow, False, False, not player_attacker)
                            ...
                            special_moves_defender.name.lower() == 'solarbeam' or special_moves_defender.name.lower() == 'hyper beam') and count_move_defender == 1:
                        show_screen_elements.explosion_animations(BattleWindow, False, False, not player_attacker)
                    else:
                        show_screen_elements.wait(BattleWindow, 30, False, False)
                else:
        ...
        if critic_value > 0.98 and (
                move.kind == "Physical" or move.kind == "Special") and effectiveness != 'not effect':
        ...
        # stat changing moves.
            ...
            if move.kind == "a+":
                ...
                attacker_model.team[0].battleATK = attacker_model.team[0].originalATK * utils.statMod(
                    attacker_model.team[0].atkStage)
                BattleWindow.description_battle = attacker_model.team[0].name + '\'s attack rose!'

            elif move.kind == "d+":
                attacker_model.team[0].defStage += 1
                attacker_model.team[0].battleDEF = attacker_model.team[0].originalDEF * utils.statMod(
                    attacker_model.team[0].defStage)
                ...
                defender_model.team[0].battleATK = defender_model.team[0].originalATK * utils.statMod(
                    defender_model.team[0].atkStage)
                BattleWindow.description_battle = defender_model.team[0].name + ' attacks fell!'

            elif move.kind == "d-":
                defender_model.team[0].defStage -= 1
                defender_model.team[0].battleDEF = defender_model.team[0].originalDEF * utils.statMod(
                    defender_model.team[0].defStage)
        ...
        if (move.kind == "Physical" or move.kind == 'Special') and hp_lost != 0:
            ...
            if move.name == 'Giga Drain':
                BattleWindow.description_battle = attacker_model.team[0].name + ' gained ' + str(
                    abs(gain_hp)) + ' HP and ' + \
...
def pokemon_fainted(BattleWindow, player_me_attacker, pokemon):
    ...
    if player_me_attacker:
        ...
        if index != -1:
            BattleWindow.model.enemy.swap_position(0, index)
            BattleWindow.description_battle = 'The foe chooses ' + BattleWindow.model.enemy.team[0].name + '.'
            BattleWindow.pokemon_enemy_visible = False
            show_screen_elements.pokeball_animations(BattleWindow, 10, False, False, False,
                                                     BattleWindow.model.me.team[0],
                                                     BattleWindow.model.enemy.team[0], False, True)
            show_screen_elements.pokeball_animations(BattleWindow, 10, False, False, True,
                                                     BattleWindow.model.me.team[0],
                                                     BattleWindow.model.enemy.team[0], False, True)
