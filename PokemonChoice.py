...
import random
from UI.PokemonChoiceUI import Ui_MainWindow
...
class PokemonChoice(QMainWindow):
    """
    PokemonChoice class used to view all Pokemon with their stats and select a team to start the game.
    """
    def __init__(self, model: Model):
        super(PokemonChoice, self).__init__()
    ...
    def battle(self):
        ...
        if len(self.model.me.team) == 6:
            ...
            msg_box.setText("You have to choose six Pokemon to start a battle.")
            msg_box.exec_()
...
if __name__ == "__main__":
    ...
    model = Model()
    choose_pokemon = PokemonChoice(model)
    choose_pokemon.show()
    sys.exit(app.exec_())
